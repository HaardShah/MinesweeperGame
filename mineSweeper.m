...
% The player will be asked if they are ready to play one more time, just in
% case they feel like not playing anymore :-(
...
for i = 1:3
   ...
   % The following vectors are used to prevent the user from entering the
   % same row column combination again. Then a for loop is created, which
   % will take in the row and column entered by the player, and concatenate
   % into their respective row and column vectors. If the row and column
   % are recognised to be of the same combination, the player will be asked
   % to enter them again. This will be continously asked until a different
   % row column combination is acheived.
    repRowChecker = [repRowChecker, sr];
    repColumnChecker = [repColumnChecker, sc];
    for i = 2:length(repRowChecker)
            while repRowChecker(i-1) == sr && repColumnChecker(i-1) == sc 
                fprintf("\n");
                fprintf("                                                              shots left: %d \n", count);
                disp("Error. You have already selected that combination.");
                    sr = input("Please enter a different row: ");
                    sc = input("Please enter a different column: ");
            end
    end
